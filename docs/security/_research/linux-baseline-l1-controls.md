...
| `os-03b` | Check passwords hashes in /etc/passwd | *none in source* | Open | Not asserted. Kairos writes no password hash into `/etc/passwd`: the `kairos` user is created with `passwd: "!"` through yip's `users` (`kairos-init/pkg/bundled/cloudconfigs/10_accounting.yaml:14`) and root is locked with `passwd -l root` (`:42`), both of which land in `/etc/shadow`. De facto satisfied, but nothing in the repo checks it. |
