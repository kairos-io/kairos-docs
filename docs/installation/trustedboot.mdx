...
If you want to learn more on what Trusted Boot is and how it works, see the [Trusted Boot Architecture](/docs/architecture/trustedboot) page. This page describes how to enable Trusted Boot support in Kairos.
...
The installation process is performed as usual and the [Installation instructions](/docs/installation/) can be followed, however the difference is that user-data will be automatically encrypted (both the OEM and the persistent partition) by using the TPM chip and the Trusted Boot mechanism.
...
See the [Trusted Boot Upgrade](/docs/upgrade/trustedboot) page.
...
FROM fedora
RUN dnf install -y edk2-ovmf qemu swtpm wget
WORKDIR /
