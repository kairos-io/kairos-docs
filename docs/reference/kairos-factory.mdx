...
The Kairos factory is a single step process applied on a container image. All you need to do is run [kairos-init](https://github.com/kairos-io/kairos/tree/master/kairos-init) in your Dockerfile. Optionally, you can use [auroraboot.md](auroraboot.md) to generate artifacts (isos, raw images, etc..) based on the generated OCI artifact.
...
| --skip-step                 | Skip the given steps during the image build                       | Steps or Stages            | None            |
...
We recognize that one size does not fit all, and sometimes you may want to skip certain steps or stages during the image build process. To accommodate this, kairos-init provides the `--skip-step` flag. 
...
Run `kairos-init steps-info` to see the available steps and their descriptions. You can pass more than one step, separated by comma, to skip multiple steps, for example: `--skip-step installPackages,kernel`.
...
FROM ubuntu:24.04
RUN --mount=type=bind,from=kairos-init,src=/kairos-init,dst=/kairos-init /kairos-init -l debug -s install --version "v0.0.1" --skip-step installKernel
...
For users who prefer to automate their Kairos image builds using GitHub Actions, Kairos ships [`reusable-factory.yaml`](https://github.com/kairos-io/kairos/blob/master/.github/workflows/reusable-factory.yaml), a reusable workflow that simplifies the build process.

It used to live in `kairos-io/kairos-factory-action`. That repository was archived on 2026-08-26 and the workflow moved into the `kairos-io/kairos` monorepo, so the `uses:` lines below name the monorepo. If your workflows still call the old repository, rewrite them:

```bash
find .github/workflows -type f \( -name '*.yaml' -o -name '*.yml' \) -exec sed -i \
  's|kairos-io/kairos-factory-action/\.github/workflows/|kairos-io/kairos/.github/workflows/|g' {} +
```

The inputs did not change, so that rewrite plus a ref is the whole migration. The old repository still resolves, but it takes no new commits, and Renovate stops proposing bumps for a `uses:` line that points at an archived repository.
...
jobs:
  build:
    uses: kairos-io/kairos/.github/workflows/reusable-factory.yaml@v4.3.0
    with:
      version: "v1.0.0"
...
The examples pin at `v4.3.0`, the first Kairos release that carries the workflow. Every subsequent release tags it again, so a tag pin follows the Kairos release cadence. Two other refs are valid:

- `@<sha>` pins at one commit and is the pattern Kairos itself uses for third-party actions. Renovate keeps it current.
- `@master` always runs the newest version, at the cost of reproducible builds.

Nothing older than `v4.3.0` has the file, so do not pin at `v4.2.0` or earlier.
...
If your base image lives on a registry served over plain HTTP, or one that presents an untrusted or self-signed TLS certificate (for example a local registry on a self-hosted runner), set the `allow_insecure_registries` input. It is passed to AuroraBoot as `--allow-insecure-registries` when generating the artifacts (ISO, UKI, and RAW).
...
jobs:
  build:
    ...
    with:
      version: "v1.0.0"
      base_image: "my-registry.internal:5000/myorg/ubuntu:24.04"
      model: "generic"
      iso: true
      allow_insecure_registries: true
```

This input affects only the pulling of images while building the artifacts; there is no corresponding option for pushing.

:::caution
Disabling TLS verification removes protection against man-in-the-middle attacks. Use it only in controlled environments, not against public registries.
:::

Every input is documented in the `workflow_call` block at the top of [`reusable-factory.yaml`](https://github.com/kairos-io/kairos/blob/master/.github/workflows/reusable-factory.yaml), which is the authoritative list.
