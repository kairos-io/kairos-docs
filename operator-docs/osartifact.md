...
7. **Pulling the base image from, or pushing the built image to, a registry that is either plain HTTP or HTTPS with a self-signed/untrusted certificate?** → Set `spec.image.pullInsecureRegistry: true` and/or `spec.image.pushInsecureRegistry: true`. See [Insecure registries](../private-registries/#insecure-registries).
