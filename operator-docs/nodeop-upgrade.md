...
- When the NodeOpUpgrade reconciles, it lists nodes matching `spec.nodeSelector`, compares each node's `kairos.io/image-repo` annotation to `spec.image`, and pre-records the matching ones in `nodeOpUpgrade.status.nodeStatuses` as `Phase: Completed` with a `Skipped: node already at image …` message. The underlying NodeOp's `nodeSelector` is augmented with a `kubernetes.io/hostname NotIn [...]` expression so it never targets those nodes — no Pod is scheduled there.
- If **every** matching node is already at `spec.image`, no NodeOp is created at all and the NodeOpUpgrade settles directly to `Phase: Completed`.
- If a node has no `kairos.io/image-repo` annotation (for example, an older Kairos that does not write `KAIROS_IMAGE_REPO`, or before the labeler has had a chance to run), it is **not** skipped and goes through the normal upgrade flow. An in-Pod version check (comparing `KAIROS_VERSION`/`KAIROS_SOFTWARE_VERSION` between the image and the host) remains as a backstop and will still exit early without performing an upgrade if the versions match.
- The annotation comparison is a literal string match against `spec.image`, so alternative references for the same content (digest pin, mirror alias, different tag pointing at the same digest) will **not** trigger the pre-Pod skip. The in-Pod version check still applies on those nodes.

Set `spec.force: true` to disable both layers and run the upgrade on every targeted node regardless of the annotation or the in-Pod version check. This is the right escape hatch when you want to re-run an upgrade with the same image but with different upgrade flags.
...
3. Before the NodeOp is created, the upgrade controller filters out nodes whose `kairos.io/image-repo` annotation already matches `spec.image` (unless `spec.force: true`). Those nodes are pre-recorded in `nodeOpUpgrade.status.nodeStatuses` as `Completed` with a skip message and the NodeOp's `nodeSelector` is augmented to exclude them — see [Skipping nodes already at the target image](#skipping-nodes-already-at-the-target-image).
