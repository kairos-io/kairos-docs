...
HUGO_PUBLIC_DIR="${ROOT_DIR}/public"
...
collect_hugo_generated_root_urls() {
  local generated_url generated_file page_id
  ...
  # If generated files are present in Hugo public output, map them so content can
  # be compared file-by-file against Docusaurus static output.
  for generated_url in /robots.txt /llms.txt /index.json /sitemap.xml; do
    generated_file="${HUGO_PUBLIC_DIR}${generated_url}"
    if [[ -f "${generated_file}" ]]; then
      page_id="generated${generated_url}"
      hugo_pages["${page_id}"]="${generated_file}"
      hugo_url_to_page["${generated_url}"]="${page_id}"
      hugo_urls["${generated_url}"]=1
    fi
  done
}
...
collect_docusaurus_urls() {
  ...
  # Generated by Docusaurus llms-txt plugin at build time.
  docusaurus_urls["/llms.txt"]=1
...
normalize_content() {
  ...
  ' "${file_path}" | FILE_PATH="${file_path}" perl -0777 -ne '
    ...
    sub canonical_doc_ref {
      ...
      $value =~ s#^(?:(?:\./)|(?:\.\./))+##;
      $value =~ s#^docs/##i;
      $value =~ s#^quickstart/##i;
      $value =~ s#^getting-started/##i;
      $value =~ s#/$##;
      $value = lc($value);
      if ($value eq q{}) {
        return q{index};
      }
      if ($value eq q{troubleshooting}) {
        return q{reference/troubleshooting};
      }
      if ($value eq q{immutable}) {
        return q{architecture/immutable};
      }
    ...
    $text =~ s/\bhref=(["\x27])(?!https?:|mailto:|#|__DOCREF__)(\/[^"\x27]+)\1/"href=$1__DOCREF__[" . canonical_doc_ref($2) . "]$1"/gei;
    ...
    # Treat JSON-LD script variants (raw JSON block vs MDX dangerouslySetInnerHTML)
    # as equivalent for migration checks.
    $text =~ s#<script\b(?=[^>]*type\s*=\s*["'\'']application/ld\+json["'\''])[\s\S]*?</script>#__JSONLD__#gmi;
