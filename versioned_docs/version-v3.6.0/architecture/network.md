...
:::tip Note
You can also watch our [Kairos and libp2p video](/docs/v3.6.0/media/#how-kairos-uses-libp2p "Media") in the [Media Section](/docs/v3.6.0/media/ "Media")
:::
...
- The Kairos base OS with support for different distribution flavors and k3s combinations (see our support matrix [here](/docs/v3.6.0/reference/image_matrix/)).
...
- For instance, with [KubeVIP](/docs/v3.6.0/examples/multi-node-p2p-ha-kubevip/), new nodes can join the network and become cluster members even after the cluster provisioning phase, making EdgeVPN a scalable solution.
...
It's also worth noting that while a VPN provides a unified network environment, it may not be necessary or appropriate for all use cases. Users can choose to opt-out of using the VPN and leverage only the coordination aspect, for example, with KubeVIP. Ultimately, the decision to use a VPN should be based on the specific needs and requirements of your Kubernetes cluster, and as such you can just use the co-ordination aspect and leverage for instance [KubeVIP](/docs/v3.6.0/examples/multi-node-p2p-ha-kubevip/).
...
A set of Kubernetes Native Extensions ([Entangle](/docs/v3.6.0/reference/entangle/)) provides peer-to-peer functionalities also to existing clusters by allowing to bridge connection with the same design architecture described above.
...
See also the Entangle [documentation](/docs/v3.6.0/reference/entangle/) to learn more about it.
