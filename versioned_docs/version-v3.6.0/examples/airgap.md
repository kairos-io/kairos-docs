...
If you want to create an [airgap K3s installation](https://docs.k3s.io/installation/airgap), Kairos provides a convenient way to do so using AuroraBoot. In this guide, we will go through the process of creating a custom ISO of Kairos that contains a configuration file and a [bundle](/docs/v3.6.0/advanced/bundles/) that executes preparatory steps after installation. The bundle will overlay new files in the system and prepare the node for having an airgapped K3s installation.
...
2. Build the ISO with [AuroraBoot](/docs/v3.6.0/reference/auroraboot/) by running the following command:


```bash
IMAGE={{< oci variant="standard" kairosVersion="v3.6.0" k3sVersion="v1.33.5+k3s1" >}}
...
- [Customize the OS image](/docs/v3.6.0/advanced/customizing/)
- [Create ISOs with Kubernetes](/docs/v3.6.0/installation/automated/#kubernetes)
- [Bundles reference](/docs/v3.6.0/advanced/bundles/)
- [System extensions](/docs/v3.6.0/advanced/sys-extensions/)
