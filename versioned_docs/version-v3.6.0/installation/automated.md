...
contains an [`edgevpn` token](https://github.com/mudler/edgevpn/blob/master/docs/content/en/docs/Concepts/Token/_index.md). For more information, [check out the architecture section](/docs/v3.6.0/architecture/network/).
...
It is possible to create custom ISOs with an embedded cloud configuration. This allows the machine to automatically boot with a pre-specified configuration file, which will be installed on the system after provisioning is complete. See also [AuroraBoot](/docs/v3.6.0/reference/auroraboot/) for documentation.
...
We can use [AuroraBoot](/docs/v3.6.0/reference/auroraboot/) to handle the the ISO build process, for example:
...
$ docker run -v $PWD:/cOS -v /var/run/docker.sock:/var/run/docker.sock -i --rm quay.io/kairos/auroraboot:v0.13.0 --debug build-iso --name "custom-iso" --date=false --overlay-iso /cOS/files-iso --output /cOS/ $IMAGE
```
</TabItem>
</Tabs>
...
You can as well modify the image in this step and add additional packages before deployment. See [customizing the system image](/docs/v3.6.0/advanced/customizing/).

Check out the [AuroraBoot documentation](/docs/v3.6.0/reference/auroraboot/) and the [examples](/docs/v3.6.0/examples/) for learn more on how to generate customized images for installation.
...
You can read more about it [here](/docs/v3.6.0/advanced/build/#build-an-iso).
