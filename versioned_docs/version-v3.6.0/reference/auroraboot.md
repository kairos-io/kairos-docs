...
:::info Note
This is the reference documentation for AuroraBoot. For a complete guide on creating custom cloud images, including how to use AuroraBoot in the context of a full workflow, see [Creating Custom Cloud Images](/docs/v3.6.0/advanced/creating_custom_cloud_images/).
:::
...
docker run --rm -ti -v "$PWD"/config.yaml:/config.yaml -v ${PWD}:/tmp quay.io/kairos/auroraboot \
                    --set "artifact_version=v3.6.0" \
                    --set "release_version=v3.6.0" \
...
1. Select the release of Kairos that you want to deploy and optionally a cloud config (see also our [examples](/docs/v3.6.0/examples/))
...
To use a container image, you can use [the Kairos released images](/docs/v3.6.0/reference/image_matrix/) or [customized](/docs/v3.6.0/advanced/customizing/) by specifying `--set container_image` instead with the container image of choice.

#### 2. Run AuroraBoot

Now we can run AuroraBoot with the version we selected, either from GitHub releases or directly from a container image.

In the example below we selected <code>v3.6.0-<K3sVersion /></code>, <FlavorCode /> flavor, so we would run either one of the following:
...
You can use [the Kairos released images](/docs/v3.6.0/reference/image_matrix/) or [your own](/docs/v3.6.0/advanced/customizing/).
...
```bash
docker pull {{< OCI variant="standard" kairosVersion="v3.6.0" k3sVersion="v1.33.5+k3s1"  >}}
...
docker run --rm -ti -v /var/run/docker.sock:/var/run/docker.sock --net host quay.io/kairos/auroraboot \
                    --set "container_image=oci:{{< OCI variant="standard" kairosVersion="v3.6.0" k3sVersion="v1.33.5+k3s1"  >}}"
```
</TabItem>
...
```bash
docker run --rm -ti --net host quay.io/kairos/auroraboot \
                    --set "artifact_version=v3.6.0-{{< K3sVersionOCI  >}}" \
                    --set "release_version=v3.6.0" \
                    --set "flavor={{< FlavorCode  >}}" \
                    --set "flavor_release={{< FlavorReleaseCode  >}}" \
...
Create a cloud config file, see [our documentation](/docs/v3.6.0/examples/) for ready-to use examples, but a minimal configuration that automatically installs, and allows us to login afterward can be the following:
...
docker run -v "$PWD"/config.yaml:/config.yaml \
                    ...
                    --set container_image={{< OCI variant="core" kairosVersion="v3.6.0"  >}} \
                    --set "disable_http_server=true" \
                    --set "disable_netboot=true" \
                    --cloud-config /config.yaml \
...
| `flavor`              | The Kairos flavor to use. See [the Kairos support matrix](/docs/v3.6.0/reference/image_matrix/) for a list.                                                                                                                                                                                     |
...
docker run -v "$PWD"/config.yaml:/config.yaml \
             -v "$PWD"/build:/tmp/auroraboot \
             --rm -ti quay.io/kairos/auroraboot \
             --set container_image=oci:{{< OCI variant="core" kairosVersion="v3.6.0"  >}} \
             --set "disable_http_server=true" \
             --set "disable_netboot=true" \
             --cloud-config /config.yaml \
             --set "state_dir=/tmp/auroraboot"
...
See the [Airgap example](/docs/v3.6.0/examples/airgap/) in the [examples section](/docs/v3.6.0/examples/).
...
```yaml
container_image: "{{< OCI variant="core" kairosVersion="v3.6.0"  >}}"
