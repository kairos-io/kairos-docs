...
Kairos is distro-agnostic by design. Currently, you can pick among a list from the [supported matrix](/docs/v3.6.0/reference/image_matrix/#image-flavors), but we are working on CRDs to let assemble OSes from other bases in a Kubernetes native way.
...
Kairos is distribution agnostic. It supports all the distributions in the [supported matrix](/docs/v3.6.0/reference/image_matrix/#image-flavors). In addition, we plan to have K3s automatically deploy Kubernetes (even by self-coordinating nodes).
