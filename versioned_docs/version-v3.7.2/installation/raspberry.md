...
docker run --rm --privileged -v /var/run/docker.sock:/var/run/docker.sock \
                ...
                --set "container_image={{< OCI variant="standard" model="rpi4" arch="arm64" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3"  >}}"
```
...
sudo dd if=build/{{< Image variant="standard" model="rpi4" arch="arm64" suffix=".raw" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3"  >}} of=<device> oflag=sync status=progress bs=10M
```
...
Extract the `img` file from a container image as described [in this page](/docs/v3.7.2/reference/image_matrix/)
...
```bash {class="only-flavors=openSUSE+Leap-15.6,openSUSE+Tumbleweed,Ubuntu+20.04,Ubuntu+22.04,Alpine+3.19"}
sudo dd if={{< Image variant="standard" model="rpi4" arch="arm64" suffix=".raw" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3"  >}} of=<device> oflag=sync status=progress bs=10MB
```
...
You can push additional `cloud config` files. For a full reference check out the [docs](/docs/v3.7.2/reference/configuration/) and also [configuration after-installation](/docs/v3.7.2/advanced/after-install/)
