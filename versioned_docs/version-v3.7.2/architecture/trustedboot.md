...
The process to generate the installable medium is described in the [Trustedboot installation documentation](/docs/v3.7.2/architecture/trustedboot/). Internally we rely on the *osbuilder* tool to generate all the installation artifacts from a single container image.
...
4. The initrd will decrypt the user-data and mount the portions of it in the root filesystem. This includes for instance any changes to `/etc` (like adding new users and passwords), `/usr/local`, and all the mount bindpoints specified in the configuration file (see [Bind mount documentation](/docs/v3.7.2/advanced/customizing/#bind-mounts) ). There is no second stage loaded and no system pivoting, the system is booted directly from the UKI file.

### Booting system

![Trusted boot](https://github.com/kairos-io/kairos-docs/assets/2420543/757870d3-3b40-46ea-9c86-13c4a545f167)

There is no difference with a layout of a standard Kairos system (as explained in the [Immutable OS](/docs/v3.7.2/architecture/immutable/) page), however in this setup now the partitions containing data are always encrypted:

- [List of persistent data path overlayed and encrypted](https://github.com/kairos-io/packages/blob/528682cddf7191fb52580e7c41a33e73c1ee0001/packages/static/kairos-overlay-files/files/system/oem/00_rootfs_uki.yaml#L18) (see also [the bind mount documentation](/docs/v3.7.2/advanced/customizing/#bind-mounts) to customize it with your own paths).
- `/oem` encrypted
- `/usr/local` encrypted
- `/etc` ephemeral
- `/usr` read only
...
Check the relevant documentation on how to [extend the system with system extensions](/docs/v3.7.2/advanced/sys-extensions/) and how to [use /opt with system extensions](/docs/v3.7.2/advanced/adding_opt_to_system_extensions/).

### Trusted Boot - Boot Assessment

See the [Trusted Boot - Boot Assessment](/docs/v3.7.2/examples/boot_assessment_trusted_boot/) documentation for more information on how to enable automatic boot assessment with Trusted Boot and how to make your own services participate in the boot assessment process.
...
For more information about installing Kairos with Trusted Boot, see the [Trusted Boot Installation Guide](/docs/v3.7.2/architecture/trustedboot/).

For more information about customizing Kairos images, see the [Customizing Images](/docs/v3.7.2/advanced/customizing/) guide.

For more information about the immutable nature of Kairos, see the [Immutable Architecture](/docs/v3.7.2/architecture/immutable/) guide.

For more information about system extensions, see the [System Extensions](/docs/v3.7.2/advanced/sys-extensions/) guide.

For more informations about revoking certificates, see [Revoking secure boot access](/docs/v3.7.2/advanced/revoking-secureboot-access/)

For a complete example of boot assessment with Trusted Boot, see the [Boot Assessment with Trusted Boot](/docs/v3.7.2/examples/boot_assessment_trusted_boot/) guide.
