...
See the [Trusted Boot Installation](/docs/v3.7.2/installation/trustedboot/) and [Trusted Boot Architecture](/docs/v3.7.2/architecture/trustedboot/) pages for more references.
...
docker run --rm -v $PWD/keys:/keys -v $PWD:/work -ti quay.io/kairos/auroraboot:v0.14.0 build-uki -t container --public-keys /keys --tpm-pcr-private-key $PATH_TO_TPM_KEY --sb-key $PATH_TO_SB_KEY --sb-cert $PATH_TO_SB_CERT $CONTAINER_IMAGE
```
...
To upgrade Kairos with Kubernetes, the Kairos operator needs to be deployed on the target cluster. [Read the instructions here](/docs/v3.7.2/upgrade/kairos-operator/).
...
:::tip Note
To understand more on how this works, see the [Kairos operator documentation](/docs/v3.7.2/upgrade/kairos-operator/) for general information about the operator and the [regular upgrade process](/docs/v3.7.2/upgrade/kubernetes/) for non-trusted boot upgrades.
:::
