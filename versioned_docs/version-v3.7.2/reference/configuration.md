...
Here's an example that shows how to set up DNS at the [boot stage](/docs/v3.7.2/architecture/cloud-init/) using the extended syntax:
...
$ docker run -ti -v $PWD:/test --entrypoint /usr/bin/kairos-agent --rm {{< OCI variant="core" kairosVersion="v3.7.2"  >}} run-stage --cloud-init-paths /test initramfs
...
This configuration can be either manually copied over, or can be propagated also via Kubernetes using the system upgrade controller. See [the after-install](/docs/v3.7.2/advanced/after-install/) section for an example.
...
The `stages` key is a map that allows to execute blocks of cloud-init directives during the lifecycle of the node [stages](/docs/v3.7.2/architecture/cloud-init/).
...
Read more about them in this page: [Stage modules](/docs/v3.7.2/reference/stage_modules/)
