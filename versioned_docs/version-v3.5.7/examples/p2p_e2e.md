...
We will first create a cloud config file for our deployment, and then run [AuroraBoot](/docs/v3.5.7/reference/auroraboot/) locally. We then start 2 VMs configured for netbooting.
...
We now can run [AuroraBoot](/docs/v3.5.7/reference/auroraboot/) with <OCICode variant="standard" /> to provision `openSUSE Leap` machines with those k3s and kairos versions.
...
Start the Machines (VM, or baremetal) with Netboot ( see also [here](/docs/v3.5.7/reference/auroraboot/#3-start-nodes) ) and wait for the installation to finish.
...
By default, the Kubernetes API endpoint is not exposed outside the VPN. This is an opinionated configuration from Kairos. To check out configurations without VPN, see also [the KubeVIP example](/docs/v3.5.7/examples/multi-node-p2p-ha-kubevip/).
...
- [Installation with p2p](/docs/v3.5.7/installation/p2p/)
- [P2P Architecture](/docs/v3.5.7/architecture/network/)
