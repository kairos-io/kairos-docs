...
We are going to see how to do that manually or with Kubernetes by using the [Kairos operator](/docs/v3.5.7/upgrade/kairos-operator/).
...
To push configurations to a node, it is recommended to use the [Kairos operator](/docs/v3.5.7/upgrade/kairos-operator/) which provides a more integrated approach for managing Kairos nodes. In the example below, we use a NodeOp to push a swapfile configuration and restart the node afterward.
