...
contains an [`edgevpn` token](https://github.com/mudler/edgevpn/blob/master/docs/content/en/docs/Concepts/Token/_index.md). For more information, [check out the architecture section](/docs/v3.5.7/architecture/network/).
...
It is possible to create custom ISOs with an embedded cloud configuration. This allows the machine to automatically boot with a pre-specified configuration file, which will be installed on the system after provisioning is complete. See also [AuroraBoot](/docs/v3.5.7/reference/auroraboot/) for documentation.
...
We can use [AuroraBoot](/docs/v3.5.7/reference/auroraboot/) to handle the the ISO build process, for example:
...
You can as well modify the image in this step and add additional packages before deployment. See [customizing the system image](/docs/v3.5.7/advanced/customizing/).

Check out the [AuroraBoot documentation](/docs/v3.5.7/reference/auroraboot/) and the [examples](/docs/v3.5.7/examples/) for learn more on how to generate customized images for installation.
...
You can read more about it [here](/docs/v3.5.7/advanced/build/#build-an-iso).
