...
{{% alert color="info" %}}
The persistent data live in directories that are bind mounted to `/usr/local` (check the [bind_mounts configuration option]({{< relref "../reference/configuration" >}})). Creating a directory in `/usr/local` manually (e.g. using a command in some `stage`) won't work.
{{% /alert %}}
