...
| `iso.overlay_uefi`    | Path to a directory whose contents will be added to the UEFI boot image inside the ISO. See [Customizing ISO contents](#customizing-iso-contents).                                                                                                                                                    |
...
| `iso.overlay_uefi` / `--overlay-uefi` | UEFI boot image | EFI boot partition | No | Top-level config / `--set`, `build-iso` |
...
#### `--overlay-uefi` - Add files to the UEFI boot image

Adds the contents of a directory to the UEFI boot image (the EFI system partition image embedded in the ISO). This is a specialized option for cases where you need to customize the EFI boot process, such as adding custom EFI binaries or boot configurations.

This option is available as a CLI flag on the `build-iso` subcommand, and as `iso.overlay_uefi` in the top-level YAML configuration. It is **not** available in `build-uki`.

```bash
docker run -v "$PWD"/uefi-overlay:/uefi-overlay \
             -v "$PWD"/build:/tmp/auroraboot \
             --rm -ti quay.io/kairos/auroraboot \
             build-iso --overlay-uefi /uefi-overlay \
             --output /tmp/auroraboot $IMAGE
```
