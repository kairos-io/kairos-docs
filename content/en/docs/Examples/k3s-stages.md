...
This tutorial is based on Debian Bookworm. Unit file configurations vary over distributions and we are not able to test them all but they should be easily adaptable from this tutorial.
{{% /alert %}}

# Introduction

Some use cases require a stage that is run after the k3s servers are up, such as to apply manifests to the Kubernetes cluster, to send healthchecks, or any other use cases. Using systemd units, we are able to run a stage once a service is started, which can be used for k3s and k3s-server in order to run steps after the k3s server is up and ready to accept requests.

# Requirements

 - A custom image that adds a systemd unit file to run our own stage
...
The main step is to create an image that has the systemd units we need to run our own stages.
...
```
We use a systemd unit in order to hook into the lifecycle of the k3s service and run our stage when needed. Crucially, the `WantedBy=k3s.service` line allows our service to be automatically started when k3s is ready.
...
Once the system has been installed the `k3s-ready` service is run after the k3s service has been started and running. Tt should be in status `Inactive` after running, and the `/tmp/k3s-ready` file should exist once k3s has started.
