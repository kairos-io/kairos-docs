...
The easiest way to find the offending `.efi` file and calculate its signature is to boot the image which we
want to blacklist in a sandboxed environment.

(If that is not possible and you want to avoid booting the insecure image, you will need to extract the `.efi` file using standard linux tools (e.g. from the bootable ISO).)

From within that image, run the following command to generate an "esl" file from the bootable `.efi` artifact:

```bash
hash-to-efi-sig-list /efi/EFI/kairos/active.efi active.esl
```

(assumes you have `efitools` installed)

Now `scp` that file to the machine where you have access to the image signing keys generated by enki.
