...
- Use [AuroraBoot](/docs/v3.7.2/Reference/auroraboot/)

Generic hardware based netbooting is out of scope for this document.
Below we give instructions on how to use the Kairos release artifacts to netboot and how to use [AuroraBoot](/docs/v3.7.2/Reference/auroraboot/) to boot from network.
...
```bash
#!ipxe
set url https://github.com/kairos-io/kairos/releases/download/v3.7.2
set kernel {{< image variant="standard" suffix="-kernel" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3" >}}
set initrd {{< image variant="standard" suffix="-initrd" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3" >}}
set rootfs {{< image variant="standard" suffix=".squashfs" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3" >}}
...
[AuroraBoot](/docs/v3.7.2/Reference/auroraboot/) is a Kairos convinience tool that can be used to quickly deploy Kairos from Network with zero-touch configuration, for instance:
...
wget "https://github.com/kairos-io/kairos/releases/download/v3.7.2/{{< image variant="standard" suffix="-kernel" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3" >}}"
wget "https://github.com/kairos-io/kairos/releases/download/v3.7.2/{{< image variant="standard" suffix="-initrd" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3" >}}"
wget "https://github.com/kairos-io/kairos/releases/download/v3.7.2/{{< image variant="standard" suffix=".squashfs" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3" >}}"
...
docker run \
  -d --name pixiecore --net=host -v $PWD:/files quay.io/pixiecore/pixiecore \
    boot /files/{{< image variant="standard" suffix="-kernel" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3" >}} /files/{{< image variant="standard" suffix="-initrd" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3" >}} --cmdline="rd.neednet=1 ip=dhcp rd.cos.disable root=live:{{ ID \"/files/{{< image variant="standard" suffix=".squashfs" kairosVersion="v3.7.2" k3sVersion="v1.35.0+k3s3" >}}\" }} netboot install-mode config_url={{ ID \"/files/config.yaml\" }} console=tty1 console=ttyS0 console=tty0"
```
