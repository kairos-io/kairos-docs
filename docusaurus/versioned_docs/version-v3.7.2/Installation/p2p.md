...
This makes it easier to deploy and manage Kubernetes clusters at the Edge, saving user's time and effort, allowing them to focus on running and scaling their applications. For more information about how does it work behind the scenes, [check out the architecture section](/docs/v3.7.2/Architecture/network/).

You can find full examples in our [examples section](/docs/v3.7.2/Examples/):
- [Full end to end example to bootstrap a self-coordinated cluster with Kairos and AuroraBoot](/docs/v3.7.2/Examples/p2p_e2e/)
- [Self-coordinated K3s HA cluster with KubeVIP](/docs/v3.7.2/Examples/multi-node-p2p-ha-kubevip/)
- [Multi-node, single master setup](/docs/v3.7.2/Examples/multi-node-p2p/)
- [Multi-node, HA setup](/docs/v3.7.2/Examples/multi-node-p2p-ha/)
- [Single-node setup](/docs/v3.7.2/Examples/single-node-p2p/)
...
The network token is a shared secret available to all the nodes of the cluster. It allows the node to co-ordinate and automatically assign roles. To generate a network token, see [documentation](/docs/v3.7.2/Installation/p2p/#network_token).
