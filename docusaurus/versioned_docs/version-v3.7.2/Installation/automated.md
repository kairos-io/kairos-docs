...
contains an [`edgevpn` token](https://github.com/mudler/edgevpn/blob/master/docs/content/en/docs/Concepts/Token/_index.md). For more information, [check out the architecture section](/docs/v3.7.2/Architecture/network/).
...
It is possible to create custom ISOs with an embedded cloud configuration. This allows the machine to automatically boot with a pre-specified configuration file, which will be installed on the system after provisioning is complete. See also [AuroraBoot](/docs/v3.7.2/Reference/auroraboot/) for documentation.
...
You can as well modify the image in this step and add additional packages before deployment. See [customizing the system image](/docs/v3.7.2/Advanced/customizing/).

Check out the [AuroraBoot documentation](/docs/v3.7.2/Reference/auroraboot/) and the [examples](/docs/v3.7.2/Examples/) for learn more on how to generate customized images for installation.
...
You can read more about it [here](/docs/v3.7.2/Advanced/build/#build-an-iso).
