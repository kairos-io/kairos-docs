...
:::tip Note
Looking to upgrade from a private registry OCI image? Check the [Private registry auth](/docs/v3.6.0/Advanced/private_registry_auth/) page.
:::
...
```bash
sudo kairos-agent upgrade --source oci:{{< oci variant="standard" kairosVersion="v3.6.0" k3sVersion="v1.33.5+k3s1" >}}
```
...
```bash
sudo kairos-agent upgrade --recovery --source oci:{{< oci variant="standard" kairosVersion="v3.6.0" k3sVersion="v1.33.5+k3s1" >}}
```
...
```bash
kairos-agent upgrade --source oci:{{< oci variant="standard" kairosVersion="v3.6.0" k3sVersion="v1.33.5+k3s1" >}} --boot-entry <efi_file_name_here>
```
