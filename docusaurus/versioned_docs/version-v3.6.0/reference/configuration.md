...
Here's an example that shows how to set up DNS at the [boot stage](/docs/v3.6.0/architecture/cloud-init/) using the extended syntax:
...
$ docker run -ti -v $PWD:/test --entrypoint /usr/bin/kairos-agent --rm {{< oci variant="core" kairosVersion="v3.6.0" >}} run-stage --cloud-init-paths /test initramfs
...
This configuration can be either manually copied over, or can be propagated also via Kubernetes using the system upgrade controller. See [the after-install](/docs/v3.6.0/advanced/after-install/) section for an example.
...
As P2P is a very complex topic, we have a dedicated [P2P documentation page](/docs/v3.6.0/installation/p2p/) that explains how to use it with deep details.
...
The `stages` key is a map that allows to execute blocks of cloud-init directives during the lifecycle of the node [stages](/docs/v3.6.0/architecture/cloud-init/).
...
Read more about them in this page: [Stage modules](/docs/v3.6.0/reference/stage_modules/)
