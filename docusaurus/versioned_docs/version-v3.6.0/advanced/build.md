...
:::info Note
This guide provides detailed information about building Kairos images. For a complete guide on creating custom cloud images, including when and how to use these build methods, see [Creating Custom Cloud Images](/docs/v3.6.0/advanced/creating_custom_cloud_images/).
:::
...
metadata:
  name: hello-kairos
spec:
  imageName: "{{< oci variant="standard" kairosVersion="v3.6.0" k3sVersion="v1.33.5+k3s1" >}}"
  iso: true
  bundles:
...
Note, the CRD allows to specify a custom Cloud config file, [check out the full configuration reference](/docs/v3.6.0/reference/configuration/).
...
metadata:
  name: hello-kairos
spec:
  imageName: "{{< oci variant="core" kairosVersion="v3.6.0" >}}"
  netboot: true
  netbootURL: ...
  bundles: ...
  cloudConfig: ...
...
Since release v3.3.0, Kairos release pipeline is pushing a public image to AWS, which you can use. Read how to deploy Kairos using an AMI (the released or a custom one), in the [relevant page](/docs/v3.6.0/installation/aws/).
