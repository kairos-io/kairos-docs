...
:::info Note
This guide focuses on customizing Kairos images. For a complete guide on creating custom cloud images from scratch, including when and how to apply these customizations, see [Creating Custom Cloud Images](/docs/v3.6.0/Advanced/creating_custom_cloud_images/).
:::

Kairos is an open source, container-based operating system. To modify Kairos and add a package, you'll need to build a container image from the [Kairos images](/docs/v3.6.0/Reference/image_matrix/). Here's an example with Docker which adds `figlet`:

```dockerfile
FROM {{< oci variant="standard" kairosVersion="v3.6.0" k3sVersion="v1.33.5+k3s1" >}}
...
Step 1/3 : FROM {{< oci variant="standard" kairosVersion="v3.6.0" k3sVersion="v1.33.5+k3s1" >}}
 ---> 897dc0cddf91
...
You can use your custom image when [upgrading nodes manually](/docs/v3.6.0/Upgrade/manual/), [with Kubernetes](/docs/v3.6.0/Upgrade/kubernetes/) or [specifying it in the cloud-config during installation](/docs/v3.6.0/Examples/core/). Here's how to do it manually with the `kairos-agent` command:
...
After you have modified the kernel and initrd, you can use the kairos-agent upgrade command to update your nodes, or [within Kubernetes](/docs/v3.6.0/Upgrade/kubernetes/).
...
See [ISO customization](/docs/v3.6.0/Advanced/customizing/) above.
