...
booting on their hardware in the context of ["Trusted Boot"](/docs/v3.6.0/Architecture/trustedboot/).
...
If you followed [the instructions to create signing keys](/docs/v3.6.0/Installation/trustedboot/#key-generation), you should have a directory with a `db.pem` certificate. This is the certificate
...
As described in the ["Trusted Boot"](/docs/v3.6.0/Architecture/trustedboot/) documentation page,
...
docker run --rm -v $PWD/unpacked:/unpacked \
  -v $PWD/build:/result \
  -v $PWD/new-keys/:/keys \
  quay.io/kairos/auroraboot:v0.13.0 \
  build-uki \
  --output-dir /result \
  --keys /keys \
...
The "fallback" image will be upgraded on the next upgrade. Read the ["Container based"](/docs/v3.6.0/Architecture/container/)
