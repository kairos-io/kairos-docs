...
Here's an example that shows how to set up DNS at the [boot stage](/docs/v3.5.7/Architecture/cloud-init/) using the extended syntax:
...
This configuration can be either manually copied over, or can be propagated also via Kubernetes using the system upgrade controller. See [the after-install](/docs/v3.5.7/Advanced/after-install/) section for an example.
...
As P2P is a very complex topic, we have a dedicated [P2P documentation page](/docs/v3.5.7/Installation/p2p/) that explains how to use it with deep details.
...
The `stages` key is a map that allows to execute blocks of cloud-init directives during the lifecycle of the node [stages](/docs/v3.5.7/Architecture/cloud-init/).
...
Read more about them in this page: [Stage modules](/docs/v3.5.7/Reference/stage_modules/)
